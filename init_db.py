...
def init_database():
    ...
    if user_count == 0:
        ...
        cursor.execute(
            "INSERT INTO users (username, display_name, email, password_hash, role, karma, bio) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ('admin', 'Administrator', 'admin@example.com', admin_hash, 'admin', 500, 'Главный администратор платформы')
        )
        ...
        cursor.execute(
            "INSERT INTO users (username, display_name, email, password_hash, role, karma, bio) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ('moderator', 'Moderator', 'mod@example.com', mod_hash, 'moderator', 350, 'Модератор сообществ')
        )
        ...
        test_users = [
            ('alex_tech', 'Alex Tech', 'alex@example.com', 120, 'Программист и энтузиаст технологий'),
            ('maria_art', 'Maria Art', 'maria@example.com', 85, 'Художник-иллюстратор'),
            ('pavel_games', 'Pavel Games', 'pavel@example.com', 200, 'Геймер и стример'),
            ('olga_photo', 'Olga Photo', 'olga@example.com', 45, 'Фотограф-любитель'),
            ('denis_music', 'Denis Music', 'denis@example.com', 67, 'Музыкант и мелломан'),
            ('anna_life', 'Anna Life', 'anna@example.com', 92, 'Блогер о жизни'),
            ('sergey_dev', 'Sergey Dev', 'sergey@example.com', 156, 'Full-stack разработчик'),
            ('kate_food', 'Kate Food', 'kate@example.com', 78, 'Кулинарный блогер')
        ]
        
        test_user_ids = []
        for username, display_name, email, karma, bio in test_users:
            pw_hash = generate_password_hash('test123')
            cursor.execute(
                "INSERT INTO users (username, display_name, email, password_hash, role, karma, bio) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (username, display_name, email, pw_hash, 'user', karma, bio)
            )
        ...
        posts_data = [
            # Tech World посты
            (1, 'Новый фреймворк для веб-разработки: что вы думаете?',
            ...
            (2, 'Как начать изучать Python с нуля в 2026 году',
            ...
            (3, 'Сравнение языков программирования: Rust vs Go',
            ...
            # Art Gallery посты
            (4, 'Моя новая цифровая иллюстрация "Лесной дух"',
            ...
            (5, 'Советы по анатомии для начинающих художников',
            ...
            # Gamers Hub посты
            (6, 'Обзор новинки: Elden Ring 2 — первые впечатления',
            ...
            (7, 'Лучшие инди-игры 2025 года: моя подборка',
            ...
            (8, 'Как улучшить FPS в играх: практические советы',
            ...
            # Photography посты
            (9, 'Уличная фотография: как поймать момент',
            ...
            (10, 'Выбор камеры для начинающего фотографа',
        ...
        post_ids = []
        for i, (seq, title, content, user_id, community_id, upvotes, downvotes, comments_count) in enumerate(posts_data):
        ...
        comments_data = [
            ...
            (post_ids[0], test_user_ids[2], 'А если проект на Svelte? У Astro отличная поддержка!', post_ids[0], None, None, None, 1),
            (post_ids[0], test_user_ids[6], 'Согласен, для Svelte это отличный выбор!', None, None, None, None, 2),
            ...
            (post_ids[3], test_user_ids[1], 'Спасибо! На детали ушло около 3 дней, все остальное - композиция и цвета.', None, post_ids[0], None, None, 1),
            ...
            (post_ids[4], test_user_ids[1], 'Да, "Complete Anatomy" и "Anatomy 3D Atlas" - отличные варианты!', None, post_ids[1], None, None, 1),
            ...
            (post_ids[5], test_user_ids[2], 'Боссы сложные, но справедливые. Как и в первой части - учим паттерны и побеждаем!', None, post_ids[2], None, None, 1),
            ...
            (post_ids[8], test_user_ids[3], 'Телефоны стали хороши, но для настоящей уличной фотографии все же нужна камера - больше контроля!', None, post_ids[3], None, None, 1),
        ...
        for comment in comments_data:
            post_id, user_id, content, parent_id = comment[0], comment[1], comment[2], comment[3] if len(comment) > 3 else None
